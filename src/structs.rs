...
fn is_under_v8_ns(e: Entity) -> bool {
  is_v8_ns(e) || e.get_semantic_parent().map(is_under_v8_ns).unwrap_or(false)
}
...
fn is_class_or_struct_under_v8_ns(e: Entity) -> bool {
  (e.get_kind() == EntityKind::ClassDecl
    || e.get_kind() == EntityKind::StructDecl)
    && is_under_v8_ns(e)
}

fn is_public(e: Entity) -> bool {
  e.get_accessibility()
    .map(|acc| acc == Accessibility::Public)
    .unwrap_or(false)
}

fn get_public_fields<'tu>(e: Entity<'tu>) -> Option<Vec<Entity<'tu>>> {
  e.get_type()
    .and_then(|ty| ty.get_fields())
    .and_then(|fields| {
      if fields.iter().any(|&f| is_public(f)) {
        Some(fields)
      } else {
        None
      }
...
fn visit_class<'tu>(
  ...
  //}
  if let Some(fields) = get_public_fields(class) {
    println!("{:?}", get_flat_name(class));
    for f in fields {
      println!("  {}", f.get_name().unwrap_or_else(|| "<anon>".to_owned()));
    }
  }
  Ok(())
}
...
fn visit_translation_unit<'tu>(
  ...
  root.visit_children(|e, _| {
    ...
    if e.is_definition() && is_class_or_struct_under_v8_ns(e) {
